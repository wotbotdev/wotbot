"""Application-owned external execution state; existing graph checkpoints remain authoritative."""
...
class AgentTaskRecord(Base):
    """One assistant or raw task. Physical table names preserve existing deployments.

    ``context_id`` is the conversation's thread ID: A2A contexts are ordinary
    hidden threads (``kind='a2a'`` or ``kind='mcp_raw'``), the way jobs attach to threads through
    ...
    family: Mapped[str] = mapped_column(
        Text, nullable=False, default="assistant", server_default="assistant"
    )
    operation: Mapped[dict[str, Any]] = mapped_column(
        JSONB, nullable=False, default=dict, server_default="{}"
    )
    origin: Mapped[str] = mapped_column(Text, nullable=False, default="a2a", server_default="a2a")
...
class AgentSubscriptionRecord(Base):
