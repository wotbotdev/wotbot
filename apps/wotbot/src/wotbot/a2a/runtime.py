"""Connection-independent A2A execution over the shared graph lifecycle."""
...
from wotbot.a2a.store import ACTIVE, RESERVED, Admission, TaskStore
...
class A2ARuntime:
    ...
    async def start(self):
        ...
        try:
            ...
            raise
        logger.info("A2A started; failed %d abandoned tasks (no automatic retries)", recovered)
    ...
    async def _maintain(self):
        while True:
            ...
            try:
                ...
                logger.exception("A2A retention cleanup failed")
    ...
    async def sweep(self) -> None:
        ...
        if deleted:
            logger.info("A2A retention retired %d context(s)", deleted)
        ...
        async with self.admission_lock:
            ...
            if admission.execute:
                runner = asyncio.create_task(
                    self._run(owner, admission, request["message"]), name=f"a2a:{admission.task.id}"
                )
                self.runners[admission.task.id] = runner
                runner.add_done_callback(lambda done: self._runner_done(admission.task.id, done))
                logger.info(
                    "Admitted A2A task=%s context=%s", admission.task.id, admission.task.context_id
                )
            return admission
    ...
    def _runner_done(self, task_id, runner):
        ...
        if not runner.cancelled() and runner.exception():
            logger.error("A2A runner failed task=%s", task_id, exc_info=runner.exception())
    ...
    async def _status(self, owner, task, state, text="", data=None, pending=None, message=None):
        ...
        if message is not None:
            ...
            task.status.message.CopyFrom(agent_message(task, text, data))
        else:
        ...
        logger.info(
            "A2A task=%s context=%s state=%s", task.id, task.context_id, TaskState.Name(state)
        )
    ...
    async def _run(self, owner: str, admission: Admission, message: dict):
        ...
        try:
            ...
            failures = list(MessageToDict(task).get("metadata", {}).get("wotbotOutputErrors", []))
            ...
            logger.exception("A2A execution failed task=%s context=%s", task.id, task.context_id)
            await self._status(
                owner,
                task,
                TaskState.TASK_STATE_FAILED,
        ...
        try:
            ...
            while True:
                ...
                if event is _TRUNCATED:
                    logger.warning("A2A subscriber fell behind; truncating stream task=%s", task_id)
    ...
    async def cancel(self, owner: str, task_id: str) -> Task:
        async with self.admission_lock:
            ...
            async with self.registry.thread_lock(thread_id):
                await _finalize_interrupted_run(self.graph, thread_id)
            task = await asyncio.to_thread(self.store.get, owner, task_id)
