...
import httpx
from a2a.helpers.proto_helpers import new_data_part as _new_data_part
...
def json_part(data):
    return _new_data_part(data, media_type="application/json")
        ...
        for message in batch:
            ...
            for index, item in enumerate(items):
                if not isinstance(item, dict):
                    continue
                artifact_id = str(
                    uuid5(NAMESPACE_URL, f"wotbot:{self.task_id}:{message.tool_call_id}:{index}")
                )
                try:
                    if item.get("kind") == "web" and call["name"] == "create_web_interface":
                        artifacts.append(await self._panel(artifact_id, call["args"], item))
                    else:
                        artifacts.append(await self._file(artifact_id, item))
                except Exception:
                    logger.exception(
                        "A2A export failed task=%s artifact=%s", self.task_id, artifact_id
                    )
            ...
            if data:
                identifier = str(
                    uuid5(NAMESPACE_URL, f"wotbot:{self.task_id}:{message.tool_call_id}:result")
                )
                artifacts.append(
                    Artifact(
                        ...
                        parts=[
                            json_part(
                                {"kind": "wotbot.tool_result", "tool": call["name"], "result": data}
                            )
                        ],
                    )
                )
